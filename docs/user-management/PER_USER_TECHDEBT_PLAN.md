...
- TD-UI-003D behavior rule: when Apple Calendar is already connected, the UI should expose a save/repair action for the app-specific password, not a second reconnect action. The stored Apple password is never re-displayed; if credentials need to be repaired or rotated, the user must re-enter the app-specific password. "Reconnect" is only relevant when no Apple connection exists.
